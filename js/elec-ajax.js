...
function manageData() {
    $.ajax({
        dataType: 'json',
        url: url+'myrent/api/getElecData.php',
        data: {}
    }).done(function(data){
    	console.log("取得資料");
    	manageRow(data.rows);
...
function manageRow(data) {
	...
	$.each( data, function( key, value ) {
	    console.log(key + " " + value);
	  	rows = rows + '<tr>';
	  	rows = rows + '<td>'+ value.DASHBOARD + " " + value.	DASHB_NAME+'</td>';
	  	rows = rows + '<td>'+ value.THIS_DEGREES+'</td>';

	  	// 只取小數點第一位
		rows = rows + '<td>'+new Number(value.TIMES).toFixed(1)+'</td>';

		rows = rows + '<td>'+parseInt(value.AVG_NUM)+'</td>';		
